...
// Load environment variables from client/.env or root .env
let envConfig = {};
try {
  envConfig = dotenv.parse(fs.readFileSync('.env'));
} catch (e) {
  try {
    envConfig = dotenv.parse(fs.readFileSync('client/.env'));
  } catch (e) {}
}

const projectId = process.env.VITE_FIREBASE_PROJECT_ID || envConfig.VITE_FIREBASE_PROJECT_ID;
const apiKey = process.env.VITE_FIREBASE_API_KEY || envConfig.VITE_FIREBASE_API_KEY;
