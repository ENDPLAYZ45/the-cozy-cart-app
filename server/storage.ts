...
export async function storagePut(
  ...
  if (!useForge) {
    const localDir = path.resolve(process.env.VERCEL ? "/tmp" : process.cwd(), ".storage", path.dirname(key));
    await fs.mkdir(localDir, { recursive: true });
    const localPath = path.resolve(process.env.VERCEL ? "/tmp" : process.cwd(), ".storage", key);
